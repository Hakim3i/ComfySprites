"""Patch Make / LTX workflow export injection."""
...
def patch_video_studio_export(
    ...
    *,
    ...
    enabled = export_compress_enabled(request)

    for node_id in (VIDEO_EXPORT_AUDIO_NODE_ID, VIDEO_STUDIO_COMBINE_NODE_ID):

        node = workflow.get(node_id)

        if isinstance(node, dict):

            node.setdefault("inputs", {})["enabled"] = enabled
