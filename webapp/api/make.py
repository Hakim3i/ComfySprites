...
from ..services.generations import list_recent_photo_generations
...
def api_make_lab_history(limit: int = _HISTORY_LIMIT) -> dict[str, Any]:
    ...
    with session_scope() as session:
        ...
        items = list_recent_photo_generations(session, limit=capped)
...
def api_comfyui_job_cancel(prompt_id: str) -> dict[str, Any]:
    ...
    if cancel_make_lab_job(prompt_id):
...
def api_comfyui_job_live_preview(prompt_id: str) -> Response:
    ...
    """In-memory KSampler preview frame (not written under ``outputs/photos/``)."""
