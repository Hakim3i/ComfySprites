/* Make tab: sprite creation, generation, full-page panel */
...
export async function renderCards() {
    ...
    try {
        ...
        container.innerHTML = '<p class="empty-state">Could not load sprites. Is the server running?</p>';
        return;
    }
  ...
  <article class="card clickable-card${landscapeClass}" data-id="${c.id}">
    ...
    <div class="card-overlay-bottom">
      <h3 class="card-name">${escapeHtml(c.name || 'Sprite')}</h3>
    ...
    container.onclick = async e => {
        ...
        if (deleteBtn) {
            ...
            try { await deleteSprite(id); await renderCards(); } catch { alert('Failed to delete sprite'); }
            return;
        }
...
export function openEditModal(char) {
    ...
    if (titleEl) titleEl.textContent = 'Make Sprite';
...
export function resetModal() {
    ...
    const titleEl = document.getElementById('make-panel-title');
    if (titleEl) titleEl.textContent = 'New Sprite';
...
export async function handleCreateSubmit(e) {
    ...
    try {
        ...
        if (data.spriteType === 'object') charData.gender = null;
        ...
        hideMakePanel();
        await renderCards();
    } catch (err) { alert('Failed to save sprite: ' + err.message); }
}
...
export function setupRandomSeed() {
    ...
    if (removeBgBtn) {
        removeBgBtn.addEventListener('click', async () => {
            ...
            if (!imageUrl || typeof imageUrl !== 'string') { alert('No image to process. Generate a sprite or open one to edit first.'); return; }
