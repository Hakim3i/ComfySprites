/* Edit tab: sprite editing full-page panel with transforms, filters, source overlay, and RMBG */
...
export async function renderEditRows() {
  ...
  if (!container) return;
  let list;
  try { list = await getSprites(); } catch { container.innerHTML = '<p class="empty-state">Could not load sprites.</p>'; return; }
  if (list.length === 0) { container.innerHTML = '<p class="empty-state">No sprites yet. Create one in the Make tab.</p>'; return; }
...
function setEditModalState({ spriteId, sprite, editingEditId, mainCard, name, prompt, seed, previewHtml, saveDisabled }) {
  ...
  if (titleEl) titleEl.textContent = mainCard ? 'Edit Sprite' : (editingEditId ? 'Edit edit' : 'New edit');
...
function populateEditSourceDropdown(sprite, options = {}, mainCard) {
  ...
  if (mainUrl) {
    ...
    opt.textContent = sprite?.name || 'Sprite';
...
export function setupEditTab() {
  ...
  if (removeBgBtn && previewPlaceholder) {
    removeBgBtn.addEventListener('click', async () => {
      if (!lastEditImageUrl || typeof lastEditImageUrl !== 'string') { alert('No image to process. Open a sprite or generate an edit first.'); return; }
      const bgRadio = document.querySelector('input[name="edit-bg"]:checked');
      const colorInput = document.getElementById('edit-background-color');
